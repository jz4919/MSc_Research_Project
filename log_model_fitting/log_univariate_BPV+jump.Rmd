...
EU_HAR_in_model_bv <- comp_har_pred(23, 256, log(EU_daily2$rbpv*mu1^(-2)))$model
EU_HAR_in_model_bv$coefficients
# Compute adjusted R^2 of the fitted BV model
base::summary(EU_HAR_in_model_bv)$adj.r.squared

EU_HAR_inpred_bv <- data.frame(date=EU_daily2$date[24:257], pred=predict(EU_HAR_in_model_bv), true=log(EU_daily2$rbpv[24:257]*mu1^(-2)))
...
EU_HAR_in_model_jump <- comp_har_pred(23, 256, log(EU_daily2$jump+1e-6))$model
...
EU_dHAR_in_model_jump <- comp_dhar_pred(23, 256, log(EU_daily2$jump+1e-6))$model
...
EU_naive_in_jump <- log(EU_daily2$jump[23:256])
```

## BPV+J in-sample forecast
```{r}
# HAR_BPV + HAR_J
EU_inpred_bvJ <- data.frame(date=EU_daily2$date[24:257], true=log(EU_daily2$rvar[24:257]),
...
```{r}
# Estimate the model on full data sample
EU_HAR_in_model_adj.c <- comp_har_pred(23, 256, log(EU_daily4$adj.c))$model
...
EU_HAR_inpred_adj.c <- data.frame(date=EU_daily2$date[24:257], pred=predict(EU_HAR_in_model_adj.c), true=log(EU_daily4$adj.c[24:257]))

# Estimate the model on full data sample
EU_HAR_in_model_adj.j <- comp_har_pred(23, 256, log(EU_daily4$adj.j+1e-6))$model
EU_HAR_in_model_adj.j$coefficients
base::summary(EU_HAR_in_model_adj.j)$adj.r.squared
EU_HAR_inpred_adj.j <- data.frame(date=EU_daily2$date[24:257], pred=predict(EU_HAR_in_model_adj.j), true=log(EU_daily4$adj.j[24:257]+1e-6))

EU_dHAR_in_model_adj.j <- comp_dhar_pred(23, 256, log(EU_daily4$adj.j+1e-6))$model
EU_dHAR_in_model_adj.j$coefficients
base::summary(EU_dHAR_in_model_adj.j)$adj.r.squared
EU_dHAR_inpred_adj.j <- data.frame(date=EU_daily2$date[24:257], pred=predict(EU_dHAR_in_model_adj.j), true=log(EU_daily4$adj.j[24:257]+1e-6))

# Estimate the model Naive_J
EU_naive_in_adj.j <- log(EU_daily4$adj.j[23:256])

# HAR_adj.c + adj.j
EU_inpred_adj.cj <- data.frame(date=EU_daily2$date[24:257], true=log(EU_daily2$rvar[24:257]),
