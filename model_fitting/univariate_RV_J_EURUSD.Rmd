...
comp_har_cj_pred <- function(train_start=23, train_end, c, j){
  ...
  x_m = unlist(lapply(train_start:train_end, function(index){comp_mean(index, period=23, dat=c)}))
  ...
  pred = predict(har_rv_j, data.frame(x_d=c[train_end+1], x_w=comp_mean(train_end+1, 5, c), x_m=comp_mean(train_end+1, 23, c), jump = j[train_end+1]), interval='confidence', level=0.95)
  # Return the fitted model and the one-day forecast
  return(list(pred=pred, model=har_rv_j))
}

# Define function that computes the one-day forecast of the HAR_RV_J model
comp_har_rvj_trans_pred <- function(train_start=23, train_end, rv, j, trans_c, trans_j){
  # Obtain RV_{t+1} as the training y
  ...
  x_m = trans_c(unlist(lapply(train_start:train_end, function(index){comp_mean(index, period=23, dat=rv)})))
  ...
  pred = predict(har_rv_j, data.frame(x_d=trans_c(rv[train_end+1]), 
                                      ...
                                      x_m=trans_c(comp_mean(train_end+1, 23, rv)), 
...
comp_shar_cj_pred <- function(train_start=23, train_end, c_d, c_p, c_n, j){
  # Obtain RV_{t+1} as the training y
  ...
  x_m = unlist(lapply(train_start:train_end, function(index){comp_mean(index, period=23, dat=c_d)}))
  jump = j[train_start:train_end]
  # Fit the HAR model with the training model
  shar_cj = lm(y~x_d_p + x_d_n + x_w + x_m + jump)
  # Make prediction on RV_{t+2}
  pred = predict(shar_cj, data.frame(x_d_p=c_p[train_end+1], x_d_n=c_n[train_end+1] , x_w=comp_mean(train_end+1, 5, c_d), x_m=comp_mean(train_end+1, 23, c_d), jump = j[train_end+1]), interval='confidence', level=0.95)
...
EU_HAR_RVJ_in_model <- comp_har_cj_pred(23, 256, sqrt(EU_daily4$rvar), sqrt(EU_daily4$jump))$model
EU_HAR_RVJ_in_model$coefficients
EU_HAR_RVJ_inpred <- data.frame(date=EU_daily$date[24:257], pred=predict(EU_HAR_RVJ_in_model), true=EU_daily$rvol[24:257])
...
# Estimate the model on full data sample
EU_HAR_RVJ_in_model <- comp_har_rvj_trans_pred(23, 256, EU_daily2$rvar, EU_daily2$jump, log, function(x){log(x+1e-5)})$model
EU_HAR_RVJ_in_model$coefficients
base::summary(EU_HAR_RVJ_in_model)$adj.r.squared
EU_HAR_RVJ_inpred <- data.frame(date=EU_daily$date[24:257], pred=predict(EU_HAR_RVJ_in_model), true=log(EU_daily2$rvar[24:257]))
mean((EU_HAR_RVJ_inpred$pred-EU_HAR_RVJ_inpred$true)^2)
# Plot the HAR in-sample forecast
EU_HAR_RVJ_inpred_plot <- ggplot(data=EU_HAR_RVJ_inpred, aes(x=date)) +
  geom_line(aes(y=true, color='True RVol')) +
...
EU_HAR_RVJ_outpred <- data.frame(do.call(rbind, lapply(EU_roll_start:EU_roll_end, function(x){comp_har_cj_pred(x-train_length, x, sqrt(EU_daily4$rvar), sqrt(EU_daily4$jump))$pred})), row.names = NULL)
EU_HAR_RVJ_outpred$date = EU_daily$date[(EU_roll_start+2) : (EU_roll_end+2)]
EU_HAR_RVJ_outpred$true = EU_daily$rvol[(EU_roll_start+2) : (EU_roll_end+2)]
...
```{r}
# Estimate the model on full data sample
EU_HAR_RVCJ_in_model <- comp_har_cj_pred(23, 256, sqrt(EU_daily4$adj.c), sqrt(EU_daily4$adj.j))$model
EU_HAR_RVCJ_in_model$coefficients
EU_HAR_RVCJ_inpred <- data.frame(date=EU_daily$date[24:257], pred=predict(EU_HAR_RVCJ_in_model), true=EU_daily$rvol[24:257])
...
```{r}
# Obtain the one-day out-of-sample forecast and 0.95 CI of the rolling window
EU_HAR_RVCJ_outpred <- data.frame(do.call(rbind, lapply(EU_roll_start:EU_roll_end, function(x){comp_har_cj_pred(x-train_length, x, sqrt(EU_daily4$adj.c), sqrt(EU_daily4$adj.j))$pred})), row.names = NULL)
EU_HAR_RVCJ_outpred$date = EU_daily$date[(EU_roll_start+2) : (EU_roll_end+2)]
EU_HAR_RVCJ_outpred$true = EU_daily$rvol[(EU_roll_start+2) : (EU_roll_end+2)]
...
```{r}
# Estimate the model on full data sample
EU_HAR_BVJ_in_model <- comp_har_cj_pred(23, 256, sqrt(EU_daily4$rbpv*mu1^(-2)), sqrt(EU_daily2$jump))$model
EU_HAR_BVJ_in_model$coefficients
EU_HAR_BVJ_inpred <- data.frame(date=EU_daily$date[24:257], pred=predict(EU_HAR_BVJ_in_model), true=EU_daily$rvol[24:257])
...
```{r}
# Obtain the one-day out-of-sample forecast and 0.95 CI of the rolling window
EU_HAR_BVJ_outpred <- data.frame(do.call(rbind, lapply(EU_roll_start:EU_roll_end, function(x){comp_har_cj_pred(x-train_length, x, sqrt(EU_daily4$rbpv*mu1^(-2)), sqrt(EU_daily4$jump))$pred})), row.names = NULL)
EU_HAR_BVJ_outpred$date = EU_daily$date[(EU_roll_start+2) : (EU_roll_end+2)]
EU_HAR_BVJ_outpred$true = EU_daily$rvol[(EU_roll_start+2) : (EU_roll_end+2)]
...
```{r}
# Estimate the model on full data sample
EU_SHAR_RVJ_in_model <- comp_shar_cj_pred(23, 256, sqrt(EU_daily4$rvar), sqrt(EU_daily4$P_rvar), sqrt(EU_daily4$N_rvar), sqrt(EU_daily4$jump))$model
EU_SHAR_RVJ_in_model$coefficients
EU_SHAR_RVJ_inpred <- data.frame(date=EU_daily$date[24:257], pred=predict(EU_SHAR_RVJ_in_model), true=EU_daily$rvol[24:257])
...
```{r}
# Obtain the one-day out-of-sample forecast and 0.95 CI of the rolling window
EU_SHAR_RVJ_outpred <- data.frame(do.call(rbind, lapply(EU_roll_start:EU_roll_end, function(x){comp_shar_cj_pred(x-train_length, x, sqrt(EU_daily4$rvar), sqrt(EU_daily4$P_rvar), sqrt(EU_daily4$N_rvar), sqrt(EU_daily4$jump))$pred})), row.names = NULL)
EU_SHAR_RVJ_outpred$date = EU_daily$date[(EU_roll_start+2) : (EU_roll_end+2)]
EU_SHAR_RVJ_outpred$true = EU_daily$rvol[(EU_roll_start+2) : (EU_roll_end+2)]
