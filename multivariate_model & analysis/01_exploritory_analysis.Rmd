...
# Drop the first 25 min for every week start to get rid of over-weekend rate change
EU_dat <- lapply(EU_file_names, function(x){clean_data(x, time_int="5 mins", drop_nrow=5)}) %>% flatten()
...
AU_dat <- lapply(AU_file_names, function(x){clean_data(x, time_int='5 mins', drop_nrow=5)}) %>% flatten()
# Drop days with less than 100 rows of data (weekends)
...
## Volatility plot
```{r echo=FALSE}
ggplot(data.frame(date=EU_daily$date, EURUSD=sqrt(EU_daily$rvar), AUDUSD=sqrt(AU_daily$rvar)), 
       aes(x = date)) +
  geom_line(aes(y=EURUSD, color='EURUSD')) +
...
acf(sqrt(EU_daily$rvar), lag.max=30)
```
## Box-Cox plot
```{r}
library(MASS)
# Plot the Box-Cox normality plot
boxcox(EU_daily$rvar ~ 1)
```
## QQ plots
```{r}

qqnorm(EU_daily$rvar, main="Q-Q plot of RV")
qqline(EU_daily$rvar)

qqnorm(sqrt(EU_daily$rvar), main="Q-Q plot of sqrt(RV)")
qqline(sqrt(EU_daily$rvar))

qqnorm(log(EU_daily$rvar), main="Q-Q plot of log(RV)")
qqline(log(EU_daily$rvar))
```
